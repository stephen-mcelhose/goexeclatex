...
4. **Write the source summary page** with OKF frontmatter (see `okf` skill). Infer `type` from the content — `concept` is the fallback when no other type fits better:
   ...
   | Type        | Use when the source is…                                              |
   | ----------- | -------------------------------------------------------------------- |
   | `concept`   | An idea, topic, or explanation worth synthesizing (default)          |
   | `how-to`    | A step-by-step guide for accomplishing a specific task               |
   | `decision`  | An architectural or design decision (ADR)                            |
   | `runbook`   | An operational procedure with steps and verification                 |
   | `proposal`  | A design proposal or RFC not yet decided                             |
   | `spike`     | Research findings or investigation results                           |

   ```markdown
   ---
   type: <concept | how-to | decision | runbook | proposal | spike>
   title: <Title>
   description: <one sentence — what this page covers and why it matters>
   resource: <url-or-path if a single canonical source exists; omit otherwise>
